...
class SolarSystem {
  ...
  initializeBodies() {
    const sun = new CelestialBody("Sun", 1000, 50, 0, "#FDB813", 0);
      ...
      if (body.name !== "Sun") {
        ...
        if (distance > 0) {
          ...
          const orbitalSpeed = Math.sqrt(
            (this.gravityConstant * sunMass) / distance
          );
  ...
  updateEarthDays(timeSpan) {
    ...
    if (earth) {
      ...
      const sun = this.bodies.find((b) => b.name === "Sun");
      const sunMass = sun ? sun.getEffectiveMass() : this.sunMass;
      const earthOrbitalSpeed = Math.sqrt(
        (this.gravityConstant * sunMass) / earthDistance
      );
        ...
        if (distance > 0) {
          const force =
            (this.gravityConstant *
              body1.getEffectiveMass() *
              body2.getEffectiveMass()) /
            (distance * distance);
