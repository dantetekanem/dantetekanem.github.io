class SolarSystemController {
  ...
  createPlanetControls() {
    ...
    if (bodies.length === 0) {
      container.innerHTML =
        '<p class="text-gray-400">No celestial bodies found</p>';
      return;
    }
  ...
  initializeEventListeners() {
    ...
    anchorSelect.addEventListener("change", (e) => {
      ...
      if (selectedPlanet === "") {
        ...
        const planet = this.solarSystem.bodies.find(
  ...
  addNewStar() {
    ...
    console.log(
      `Added new star: ${starName} at (${x.toFixed(1)}, ${y.toFixed(1)})`
    );
  }
  ...
  onWheel(e) {
    ...
    this.camera.targetZoom = Math.max(
      0.01,
      Math.min(20, this.camera.targetZoom)
    );
  ...
  worldToScreen(worldX, worldY) {
    ...
    const screenX =
      (scaledX + this.camera.x) * this.camera.zoom + this.canvas.width / 2;
    const screenY =
      (scaledY + this.camera.y) * this.camera.zoom + this.canvas.height / 2;
